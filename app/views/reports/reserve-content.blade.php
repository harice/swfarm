...
@foreach ($report_o->contract as $contract)
	<h3 class="margin-top-20">{{ $contract->contract_number }}</h3>
	...
	@if (sizeof($contract->order) > 0)
		@foreach ($contract->order as $order)
			...
			<table class='margin-top-30'>
				<thead>
					<tr>
						<th class='width-15' {{$border_s}}>Producer</th>
						<th class='width-11' {{$border_s}}>Stack No.</th>
						<th class='width-12' {{$border_s}}>Bales Purchased</th>
						<th class='width-15' {{$border_s}}>Bales Delivered</th>
						<th class='width-6' {{$border_s}}>Bales Remaining</th>
						<th class='width-8' {{$border_s}}>Tons Purchased</th>
						<th class='width-6' {{$border_s}}>Tons Delivered</th>
						<th class='width-8' {{$border_s}}>Tons Remaining</th>
				...
				<tbody>
					...
					<tr>
						...
						<td {{$border_s}}>{{$productorder->balesRemaining}}</td>
						<td {{$border_s}}>{{$productorder->tons}}</td>
						<td {{$border_s}}>{{$productorder->tonsDelivered}}</td>
						<td {{$border_s}}>{{$productorder->tonsRemaining}}</td>
