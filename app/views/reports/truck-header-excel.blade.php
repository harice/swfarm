<table>
	<tr>
		<td colspan='14' align='center'><h3>Trucking Statement</h3></td>
	</tr>
	<tr>
...
<table>
	<tr>
		<td colspan='8'>Truck No.: <strong>{{ strtoupper($report_o->trucknumber) }}</strong></td>
		<td></td>
		<td colspan='2'>Statement Date:</td>
		<td colspan='3' align='right'><strong>{{ date('F d, Y') }}</strong></td>
	</tr>
	<tr>
		<td colspan='8'>{{ $report_o->account->name }}</td>
		<td></td>
		<td colspan='2'>Total Bales:</td>
		<td colspan='3' align='right'><strong>{{ number_format($report_o->bales, 0, '.', ',') }}</strong></td>
	</tr>
	<tr>
		<td colspan='8'>{{ $report_o->account->businessaddress->street }}</td>
		<td></td>
		<td colspan='2'>Total Pounds:</td>
		<td colspan='3' align='right'><strong>{{ number_format($report_o->pounds, 2, '.', ',') }}</strong></td>
	</tr>
	<tr>
		<td colspan='8'>{{ $report_o->account->businessaddress->city }}, {{ $report_o->account->businessaddress->state->state_code }} {{ $report_o->account->businessaddress->zipcode }}</td>
		<td></td>
